...
class TraineeFactoryTest {
    ...
    void shouldCreateTraineeWithEncodedPasswordAndGeneratedUsername() {
        ...
        TraineeCredentialsResponseDTO result = factory.create(dto);

        assertEquals("sofia.ramirez", result.username());
        assertEquals("rawpass", result.password());

        verify(traineeRepo).save(argThat(trainee ->
                trainee.getUsername().equals("sofia.ramirez") &&
                        trainee.getPassword().equals("encodedpass") &&
                        trainee.getIsActive()
        ));
    }
