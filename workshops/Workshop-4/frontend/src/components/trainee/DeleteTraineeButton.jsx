...
export default function DeleteTraineeButton({ username, onDelete, className = "" }) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [showConfirm, setShowConfirm] = useState(false); // Estado para la confirmación visual
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Si se proporciona una función onDelete, la llamamos
      if (onDelete) {
        onDelete();
      } else {
        // Si no, navegamos por defecto (al login)
        navigate("/");
      }
      
    } catch (err) {
      // Manejo de error
      setError("❌ Error al eliminar el perfil. Verifica tu conexión o intenta de nuevo.");
      setLoading(false);
      setShowConfirm(false);
    }
  };

  // Clases base para el botón principal
    ...
    <div className="w-full">
      ...
      {showConfirm ? (
        <div className="bg-red-500 p-3 rounded-lg text-white text-center">
          <p className="font-semibold mb-2">⚠️ ¿Estás absolutamente seguro de eliminar tu cuenta?</p>
