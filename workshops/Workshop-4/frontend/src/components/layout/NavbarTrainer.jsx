      ...
      <nav className="bg-gray-800 text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            {/* Logo de la aplicación (Color de acento VERDE para Trainer) */}
            <Link to="/trainer-dashboard" className="font-extrabold text-2xl tracking-wider text-green-400 hover:text-green-300 transition duration-150">
              <span className="mr-2">🏋️</span> GymTracker
            </Link>

            {/* Enlaces de Escritorio (Ocultos en móvil) */}
            <div className="hidden md:flex items-center space-x-6">
              <Link to={`/trainers/${username}`} className="text-gray-300 hover:text-white transition duration-150 flex items-center">
                <span className="mr-1">👤</span> Perfil
              </Link>
              <Link to={`/trainers/${username}/trainees`} className="text-gray-300 hover:text-white transition duration-150 flex items-center">
                <span className="mr-1">👥</span> Trainees
              </Link>
              <Link to={`/trainers/${username}/trainings`} className="text-gray-300 hover:text-white transition duration-150 flex items-center">
                <span className="mr-1">🗓️</span> Mis Entrenamientos
              </Link>
        ...
        <div className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'}`} id="mobile-menu">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <Link onClick={toggleMenu} to={`/trainers/${username}`} className="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">
              👤 Perfil
            </Link>
            <Link onClick={toggleMenu} to={`/trainers/${username}/trainees`} className="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">
              👥 Trainees a Cargo
            </Link>
            <Link onClick={toggleMenu} to={`/trainers/${username}/trainings`} className="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">
